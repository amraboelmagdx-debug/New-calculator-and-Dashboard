    ...
    <div className="flex justify-between gap-2 text-xs min-w-0">
      ...
      <span
        className={`font-mono tabular-nums text-right shrink-0 ${
          bold ? (isDarkMode ? 'text-neutral-200 font-medium' : 'text-slate-800 font-medium') : isDarkMode ? 'text-neutral-300' : 'text-slate-700'
        }`}
      >
        {value}
      </span>
...
export default function LiveQuoteSummary({
  ...
  return (
    <div className="space-y-4" data-testid="live-quote-summary">
      <div>
        <p className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isDarkMode ? 'text-neutral-500' : 'text-slate-500'}`}>
          Live quote
        </p>
        <p className={`text-[10px] mb-1 ${isDarkMode ? 'text-neutral-600' : 'text-slate-400'}`}>
          Quote selling price
        </p>
        {calculating ? (
          <div className={`h-9 w-full rounded animate-pulse ${isDarkMode ? 'bg-neutral-800' : 'bg-slate-200'}`} />
        ) : (
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <span className={`text-xs ${isDarkMode ? 'text-neutral-400' : 'text-slate-600'}`}>Quote margin</span>
          <span className={`text-base font-bold font-mono tabular-nums ${marginColor}`}>
            {margin.toFixed(1)}%
          </span>
        </div>
