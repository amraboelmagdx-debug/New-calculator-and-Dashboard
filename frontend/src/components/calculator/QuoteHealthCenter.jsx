...
function HealthStatus({ margin, target, floorWarning, isDarkMode }) {
  ...
  return (
    <div className={`flex items-center justify-between gap-2 rounded-lg border px-3 py-2 ${tone}`} data-testid="quote-health-status">
...
function DistributionBar({ cogs, profit, isDarkMode }) {
  ...
  return (
    <div data-testid="quote-cost-profit-distribution">
      <p className={`text-[10px] uppercase tracking-wider mb-1.5 ${muted}`}>Cost / profit distribution</p>
      <div className={`flex h-2.5 rounded-full overflow-hidden ${isDarkMode ? 'bg-neutral-800' : 'bg-slate-100'}`}>
...
function ContributorList({ title, items, accessor, isDarkMode, formatValue, danger }) {
  ...
  return (
    <div>
      <p className={`text-[10px] uppercase tracking-wider mb-1 ${muted}`}>{title}</p>
...
export default function QuoteHealthCenter({
  ...
  return (
    <div className="space-y-4" data-testid="quote-health-center">
